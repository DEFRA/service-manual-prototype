...
> [!WARNING]
> The `service-manual-prototype` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `service-manual-prototype`. To turn it
...
```bash
docker build --target development --no-cache --tag service-manual-prototype:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 service-manual-prototype:development
```
...
```bash
docker build --no-cache --tag service-manual-prototype .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 service-manual-prototype
```
...
```bash
docker run -it --entrypoint /bin/ash service-manual-prototype
```
